...
fire_names <- c(
 ...
 "LNU Lightning Complex, CA (2020)",
 "Cold Springs Complex, WA (2020)",
 "Murphy Complex, ID/NV (2007)",
 "North Complex, CA (2020)",
 "Rhea Fire, OK (2018)"
)

# Subset to testing fire (August Complex)
daily.gdf <- fired.daily %>% filter(FIRED_ID == fire_ids[7] & ig_year == 2018)
rm(fired.daily)
gc()
...
static <- ggplot() +
 ...
 geom_label(aes(x = Inf, y = Inf, label = fire_names[7]), 
            hjust = 4.1, vjust = 1.4, fill = alpha("black", 0.5), 
...
ggsave(static, file = "figures/animations/RheaFire_static.png",
...
# Animate it !
animap <- static +
 transition_states(event_day, transition_length = 2, state_length = 1) +
 labs(caption = "Burn Date: {closest_state}") + # dynamic label
...
animation <- gganimate::animate(
 animap, nframes = ndays, fps=4,
 start_pause = 0, end_pause = 20,
...
# Save GIF file.
anim_save("figures/animations/RheaFire_animation.gif")
