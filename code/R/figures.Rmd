...
p1 <- mcd64 %>% as.data.frame(xy = TRUE) %>% mutate(
 ...
 coord_sf() +
 labs(fill="Days Since Ignition", tag=expression(bold("(B)"))) +
 theme_void() +
...
p2 <- mcd64 %>% as.data.frame(xy = TRUE) %>% mutate(
 ...
 geom_sf(data=points, color=alpha("black",0.8), size=0.85, shape=18) +
 coord_sf() +
 labs(fill="Days Since Ignition", tag=expression(bold("C"))) +
 theme_void() +
 theme(legend.position="none",
       plot.title = element_text(hjust = 0.5),
...
p3 <- mcd64 %>% as.data.frame(xy = TRUE) %>% mutate(
 ...
 coord_sf() +
 labs(fill="Days Since Ignition", tag=expression(bold("D"))) +
 theme_void() +
 theme(legend.position="none",
       plot.title = element_text(hjust = 0.5),
