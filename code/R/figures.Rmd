...
```{r}
ics.fast <- ics.fast %>% filter(START_YEAR == 2020)
# Summarize number of threatened, damaged, destroyed structures by fast fires from the 209s
print("ICS-209-PLUS 2020 Only Fast Fires, Structural Impacts: ")
paste0("Total fast fires (N): ",dim(ics.fast)[1])
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
paste0("Total Structures Threatened: ", scales::comma(sum(ics.fast$STR_THREATENED_MAX, na.rm=TRUE)))
paste0("Total Structures Damaged: ", scales::comma(sum(ics.fast$STR_DAMAGED_TOTAL, na.rm=TRUE)))
...
paste0("Total Burned Area: ",sum(ics.fast$FINAL_ACRES,na.rm=T))
paste0("Percent of Burned Area: ",sum(ics.fast$FINAL_ACRES,na.rm=T)/sum(ics$FINAL_ACRES,na.rm=T)*100)
...
```{r}
# Spatial map of top 100 (West)
west <- c("CA", "OR", "WA", "NV", "ID", "MT", "WY", "UT", "AZ", "NM", "CO") # Example list
states.west <- states %>% filter(STUSPS %in% west)

# Ensure centroids and states have the same CRS
centroid1 <- st_transform(centroid1, st_crs(states.west))
centroid2 <- st_transform(centroid2, st_crs(states.west))
centroid3 <- st_transform(centroid3, st_crs(states.west))

# Filter centroids by spatial intersection with western states
centroid1_west <- st_intersection(centroid1, states.west)
centroid2_west <- st_intersection(centroid2, states.west)
centroid3_west <- st_intersection(centroid3, states.west)

f1 <- ggplot() + 
   geom_sf(data = states.west, lwd = 0.6, color = "gray20", fill = "white") +
   geom_sf(data = centroid1_west, color = "gray40", size = 0.3) +
   geom_sf(data = centroid2_west, aes(color = mx_grw_ha, size = mx_grw_ha)) +
   scale_color_viridis_c(trans = "log", option = "inferno", direction = -1,
                         breaks = c(20000, 40000, 80000, 160000),
                         labels = c("20k", "40k", "80k", "160k")) +
   labs(fill = "", color = "Maximum Single-Day Growth (ha/day)") +
   guides(
       color = guide_colourbar(
           direction = "vertical", barwidth = 0.8, barheight = 12, 
   ...
   theme(
       ...
       legend.title = element_text(size = 10, angle = 90, vjust = 0.5), # Vertical title
       legend.text = element_text(size = 9),
       legend.position = c(1.12, 0.5), # Place on the right side of the map
   ...
   ggspatial::annotation_scale(
       location = "bl", # Place scale bar in bottom-left
       width_hint = 0.2, pad_x = unit(0.5, "cm"), pad_y = unit(0.3, "cm"),
       line_width = 0.6, text_pad = unit(0.15, "cm"), height = unit(0.15, "cm")
   ) +
   ggspatial::annotation_north_arrow(
       location = "bl", # Place north arrow near the scale bar
       which_north = "true",
...
# Save it out
ggsave(f1, file = "../../figures/Figure1a_FastFires_WEST.png", dpi=300, bg="white",
...
library(cowplot)
library(ggplot2)
library(ggtext)  # For halo effect

# Subset Southern California
so_cal_bbox <- st_bbox(c(
  xmin = -1950000,
  xmax = -1300000,
  ymin = -1400000,
  ymax = -750000
), crs = st_crs(states.west))
so_california <- st_crop(states.west, so_cal_bbox)
centroid1_so_cal <- st_intersection(centroid1_west, so_california)
centroid2_so_cal <- st_intersection(centroid2_west, so_california)

# convert to acres
centroid2_west <- centroid2_west %>%
  mutate(mx_grw_ac = mx_grw_ha * 2.47105)
centroid2_so_cal <- centroid2_so_cal %>%
  mutate(mx_grw_ac = mx_grw_ha * 2.47105)

# Global min and max values for mx_grw_ac
min_value <- min(centroid2_west$mx_grw_ac, na.rm = TRUE)
max_value <- max(centroid2_west$mx_grw_ac, na.rm = TRUE)

# Main Map
main_map <- ggplot() + 
   geom_sf(data = states.west, lwd = 0.6, color = "gray20", fill = "white") +
   geom_sf(data = centroid1_west, color = "gray40", size = 0.3, show.legend = TRUE) +
   geom_sf(data = centroid2_west, aes(color = mx_grw_ac, size = mx_grw_ac)) +
   geom_sf(data = so_cal_bbox_sf, fill = NA, color = "red", linetype = "dashed", size = 0.8) + 
   scale_color_viridis_c(
       trans = "log", 
       option = "inferno", 
       direction = -1,
       limits = c(min_value, max_value),  # Global limits
       breaks = c(50000, 100000, 200000, 400000),  # Adjust breaks for acres
       labels = c("50k", "100k", "200k", "400k")
   ) +
   labs(fill = "", color = "Top-100 Fastest Fires\nMaximum Single-Day Growth (acres/day)", size = "", 
        caption = "*Grey dots represent all fast fires,\n*Colored dots represent the top 100 fastest fires (2001-2020)") + 
   guides(
       color = guide_colourbar(
           direction = "vertical", barwidth = 0.8, barheight = 13, 
           title.position = "left", title.hjust = 0.5,
           ticks.colour = NA
       ),
       size = "none"
   ...
   theme(
       plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"),
       legend.title = element_text(size = 10, angle = 90, vjust = 0.5),
       legend.text = element_text(size = 9),
   ...
   ggspatial::annotation_scale(
       ...
       width_hint = 0.15,  # Adjust size
       pad_x = unit(0.65, "cm"),  # Shift to the left
       pad_y = unit(0.6, "cm"),  # Shift downward
       text_cex = 0.7
   ) +
   ggspatial::annotation_north_arrow(
       location = "bl",  # Place near the scale bar
       which_north = "true",
       pad_x = unit(0.75, "cm"),  # Shift to align with the scale bar
       pad_y = unit(1.0, "cm"),  # Adjust upward for separation
       style = north_arrow_fancy_orienteering,
       height = unit(0.6, "cm"),
       width = unit(0.6, "cm")
   )
...
city_coords <- tibble::tibble(
  city = c("Los Angeles", "San Diego"),
  lon = c(-118.2437, -117.1611),
  lat = c(34.0522, 32.7157)
)
city_points <- st_as_sf(city_coords, coords = c("lon", "lat"), crs = 4326) %>%
  st_transform(crs = st_crs(states.west))
city_coords_transformed <- city_coords %>%
  mutate(
    x = st_coordinates(city_points)[, 1], 
    y = st_coordinates(city_points)[, 2],
    label_x = x - 95000,
    label_y = y - 45000
  )
...
# Inset Map
inset_map <- ggplot() +
   geom_sf(data = so_california, fill = "white", color = "gray20", lwd = 0.6) +
   geom_sf(data = centroid1_so_cal, color = "gray40", size = 0.3) +
   geom_sf(data = centroid2_so_cal, aes(fill = mx_grw_ac, size = mx_grw_ac), 
           shape = 21, color = alpha("black", 0.6), stroke = 0.4) +
   geom_sf(data = city_points, color = "black", size = 2.5, shape = 21, fill = "white") +
   geom_richtext(data = city_coords_transformed, 
                 aes(x = label_x, y = label_y, label = city), 
                 size = 3, 
                 fill = "white", 
                 color = "black", 
...
# CONUS inset map
# Simplified CONUS and Western US boundaries
conus_states <- states %>%
  filter(!STUSPS %in% c("AK", "HI", "PR", "VI", "GU", "MP", "AS")) # Exclude non-CONUS states
western_states <- conus_states %>%
  filter(STUSPS %in% west)

# Get the bounding box of the western U.S. in CONUS
western_bbox <- st_bbox(western_states)
western_bbox_sf <- st_as_sfc(western_bbox)

# Create the new CONUS inset map
conus_inset_map <- ggplot() +
  geom_sf(data = conus_states, fill = "white", color = "gray20", lwd = 0.3) + # CONUS boundaries
  geom_sf(data = western_bbox_sf, fill = NA, color = "red", linetype = "solid", size = 1) + # Western extent
  theme_void() +
  coord_sf(expand = FALSE)

# Adjust placement for the CONUS inset
conus_inset_map_x <- 0.032  # Place on the top-right of the main map
conus_inset_map_y <- 0.72   # Move further up
conus_inset_map_width <- 0.2
conus_inset_map_height <- 0.2

# combine the maps
final_map <- ggdraw() +
  draw_plot(main_map) +
  draw_plot(inset_map, x = inset_map_x, y = inset_map_y - 0.3, width = inset_map_width, height = inset_map_height) + # Southern California inset
  draw_plot(conus_inset_map, x = conus_inset_map_x, y = conus_inset_map_y, width = conus_inset_map_width, height = conus_inset_map_height) # CONUS inset
final_map

# Save Final Map
ggsave(final_map, file = "../../figures/Figure1a_FastFires_WEST_with_Inset.png", 
       width = 9, height = 6, units = "in", bg = "white")
```

```{r}
# get the California stats
centroid1 <- st_centroid(fired.fast)
centroid2 <- st_centroid(fired.t100)

# join to states
centroid1 <- centroid1 %>% st_join(states)
centroid2 <- centroid2 %>% st_join(states)

ca_ff <- centroid1 %>% filter(STUSPS == 'CA')
ca_ff100 <- centroid2 %>% filter(STUSPS == 'CA')
