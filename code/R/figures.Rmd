...
# Global min and max values for mx_grw_ha
min_value <- min(centroid2_west$mx_grw_ha, na.rm = TRUE)
max_value <- max(centroid2_west$mx_grw_ha, na.rm = TRUE)
...
main_map <- ggplot() + 
   ...
   labs(fill = "", color = "Top-100 Fastest Fires\nMaximum Single-Day Growth (ha/day)", size = "", 
...
inset_map <- ggplot() +
   ...
   geom_sf(data = centroid2_so_cal, aes(fill = mx_grw_ha, size = mx_grw_ha), 
...
final_map <- ggdraw() +
   draw_plot(main_map) +
   draw_plot(inset_map, x = inset_map_x, y = inset_map_y, width = 0.3, height = 0.3)
final_map
