...
daily.gdf <- fired.daily %>% filter(FIRED_ID == fire_ids[5] & ig_year == 2007)
rm(fired.daily)
gc()
...
static <- ggplot() +
 ...
 geom_label(aes(x = Inf, y = Inf, label = fire_names[5]), 
            hjust = 2.85, vjust = 1.4, fill = alpha("black", 0.5), 
...
ggsave(static, file = "figures/animations/MurphyComplex_static.png",
...
animap <- static +
 ...
 labs(caption = "Burn Date: {round(frame_time)}") + # dynamic label
...
# Save GIF file.
anim_save("figures/animations/MurphyComplex_animation.gif")
