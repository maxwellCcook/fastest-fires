...
daily.gdf <- fired.daily %>% filter(FIRED_ID == fire_ids[4] & ig_year == 2020)
rm(fired.daily)
gc()
...
static <- ggplot() +
 ...
 geom_label(aes(x = Inf, y = Inf, label = fire_names[4]), 
            hjust = 2.6, vjust = 1.4, fill = alpha("black", 0.5), 
...
ggsave(static, file = "figures/animations/ColdSpringComplex_static.png",
...
# Save GIF file.
anim_save("figures/animations/ColdSpringComplex_animation.gif")
