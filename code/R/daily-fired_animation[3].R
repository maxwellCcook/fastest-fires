...
daily.gdf <- fired.daily %>% filter(FIRED_ID == fire_ids[3] & ig_year == 2020)
rm(fired.daily)
gc()
...
static <- ggplot() +
 ...
 geom_label(aes(x = Inf, y = Inf, label = fire_names[3]), 
            hjust = 2.525, vjust = 1.4, fill = alpha("black", 0.5), 
...
ggsave(static, file = "figures/animations/LNUComplex_static.png",
...
# Save GIF file.
anim_save("figures/animations/LNUComplex_animation.gif")
